...
// ==============================================================
// KOMPONEN FORM DIPINDAHKAN KE LUAR AGAR INPUT TIDAK LOSE FOCUS
// ==============================================================
function PermintaanForm({ form, setForm, user, divisi, jabatan, jenisSurat, signers, departemenIM, onDocTypeChange, onDeptChange, onSignerChange }) {
  return (
    <>
...
export default function PermintaanPage() {
  ...
  const [form, setForm] = useState({ 
    ...
    posisiTTD: 'Kanan Bawah', namaKegiatan:'', tanggalKegiatan:'', daftarPeserta:''
  });
  ...
  const loadDropdowns = async () => {
    ...
    if (res.success) {
      setDivisi(res.divisi); setJabatan(res.jabatan); setSigners(res.signers); setJenisSurat(res.jenisSurat); setDepartemenIM(res.departemenIM); 
    }
  };
  ...
  const onDeptChange = (dept) => setForm({ ...form, departemen: dept });
  ...
  const submitForm = async () => {
    ...
    if (form.jenisSurat === 'Sertifikat') {
      if (!form.namaKegiatan || !form.daftarPeserta) {
        Swal.fire({icon:'warning', title:'Perhatian', text:'Nama Kegiatan dan Daftar Peserta wajib diisi untuk Sertifikat', confirmButtonColor:'#1d4ed8'}); return;
      }
    } else {
      if (!form.docLink || form.docLink.indexOf('docs.google.com') === -1) {
        Swal.fire({icon:'warning', title:'Perhatian', text:'Masukkan link Google Docs yang valid', confirmButtonColor:'#1d4ed8'}); return;
      }
      const confirmAccess = await Swal.fire({
        title: 'Konfirmasi Akses Dokumen', html: 'Apakah Anda sudah mengatur Sharing dokumen ke <strong>Anyone with the link = Editor</strong>?<br/><br/><small>Jika belum, QR Code tidak bisa disisipkan saat disetujui.</small>',
    ...
    try {
      const payload = {
        requesterName: form.nama || user.name, requesterUsername: user.username, division: form.divisi || user.division, position: form.jabatan || user.position,
        requestType: 'Tanda Tangan', documentType: form.jenisSurat, departemen: form.departemen || '', perihal: form.perihal || '-', targetSigner: form.tujuanTtd,
        ...
        namaKegiatan: form.namaKegiatan || '-', tanggalKegiatan: form.tanggalKegiatan || '-', daftarPeserta: form.daftarPeserta || '-', posisiTTD: form.posisiTTD || 'Kanan Bawah'
      };
      
      let res;
      if (editId) {
        res = await fetch(`/api/requests/${editId}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ documentType: form.jenisSurat, perihal: form.perihal || '-', targetSigner: form.tujuanTtd, requestType:'Tanda Tangan' }) }).then(r => r.json());
      } else {
        res = await fetch('/api/requests', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) }).then(r => r.json());
      }
      ...
      if (res.success) {
        ...
        loadRequests(user); loadDropdowns();
      } else { Swal.fire({icon:'error', title:'Gagal', text:res.message, confirmButtonColor:'#1d4ed8'}); }
    } catch (e) {
      Swal.close(); Swal.fire({icon:'error', title:'Error', text:String(e), confirmButtonColor:'#1d4ed8'});
    }
  };

  const doForward = async (id) => {
    const req = requests.find(r => r.id === id);
    Swal.fire({ title:`Teruskan ke ${req.targetSigner}?`, html:`Permintaan dari <strong>${req.requesterName}</strong> akan diteruskan.`, icon:'question', showCancelButton:true, confirmButtonText:'Ya, Teruskan', cancelButtonText:'Batal', confirmButtonColor:'#7c3aed' })
      .then(async r => {
        if (r.isConfirmed) {
  ...
  const doReject = async (id) => {
    Swal.fire({ title:'Tolak Permintaan?', text:'Status akan berubah menjadi Ditolak', icon:'warning', showCancelButton:true, confirmButtonText:'Ya, Tolak', cancelButtonText:'Batal', confirmButtonColor:'#dc2626', input:'textarea', inputLabel:'Alasan penolakan (opsional)', inputPlaceholder:'Tulis alasan...' })
      .then(async r => {
        if (r.isConfirmed) {
  ...
  const openEdit = (req) => {
    setEditId(req.id);
    setForm({ nama: req.requesterName, divisi: req.division === '-' ? '' : req.division, jabatan: req.position === '-' ? '' : req.position, jenisSurat: req.documentType === '-' ? '' : req.documentType, perihal: req.perihal === '-' ? '' : req.perihal, tujuanTtd: req.targetSigner === '-' ? '' : req.targetSigner, namaSigner: signers.find(s => s.jabatan === req.targetSigner)?.nama || '', docLink: req.fileUrl !== '-' ? req.fileUrl : '', departemen:'', posisiTTD: 'Kanan Bawah', namaKegiatan:'', tanggalKegiatan:'', daftarPeserta:'' });
    setShowForm(true);
  };
          ...
          {showForm && (
            <div className="form-card mb-4">
              ...
              <div className="form-body">
                <PermintaanForm form={form} setForm={setForm} user={user} divisi={divisi} jabatan={jabatan} jenisSurat={jenisSurat} signers={signers} departemenIM={departemenIM} onDocTypeChange={onDocTypeChange} onDeptChange={onDeptChange} onSignerChange={onSignerChange} />
                <div className="mt-4 text-end"><button className="btn-primary-custom" onClick={submitForm}><i className="bi bi-send"></i>{editId ? 'Update' : 'Kirim Permintaan'}</button></div>
              </div>
            </div>
          )}
